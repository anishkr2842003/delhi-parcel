...
<body class="hold-transition sidebar-mini">
          ...
          <div class="row">
            <div class="col-12">
              <div class="card">
                ...
                <form class="card-body" id="filterForm">
                  <div class="row">
                    <div class="col-md-3">
                    ...
                    <div class="col-md-3">
                      <div class="form-group">
                        ...
                        <select class="form-control" id="seller" name="seller">
                          ...
                          <?php
                          $seller_sql = "SELECT id, fullName FROM seller";
                          $seller_result = mysqli_query($conn, $seller_sql);
                          while ($seller_row = mysqli_fetch_assoc($seller_result)) {
                            echo "<option value='" . $seller_row['id'] . "'>" . $seller_row['fullName'] . "</option>";
                          }
                          ?>
                        </select>
                      </div>
                ...
                <div class="card-body">
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    <tbody id="orderTableBody">
                      <?php
                      $sql = "SELECT orders.*, delivery.name AS delivery_boy_name, seller.fullName AS seller_name FROM orders LEFT JOIN delivery ON orders.assign_to = delivery.id LEFT JOIN seller ON orders.seller_id = seller.id";
                      ...
                      ?>
                        <tr>
                          ...
                          <td><button type="button" class="btn btn-block <?php echo $button_class; ?> order_status" data-toggle="modal" data-target="#modal-order_status" data-orderid=<?php echo $row['order_id'] ?>><?php echo $row['order_status'] ?></button></td>
                          <td>
                            <button type="button" class="btn btn-block btn-success assign" data-toggle="modal" data-target="#modal-default" data-orderid=<?php echo $row['order_id'] ?>> <?php echo isset($row['assign_to']) ? 'Reassign' : 'Assign' ?></button>
                            <div class="badge bg-danger"><?php echo isset($row['assign_to']) ? $row['delivery_boy_name'] : '' ?></div>
                          </td>
                          <td style="width: 300px;"><?php echo $row['created_at'] ?></td>
                          <td>
                            <!-- <span class="badge bg-primary" data-toggle="modal" data-target="#modal-default"><a href="#"><i class="fas fa-edit"></i></a></span> -->
        ...
        <div class="modal-body">
          <form id="statusForm">
            <div class="card-body">
              ...
              <div class="form-group">
                <label for="orderStatus2">Select Order Status</label>
                <select class="custom-select rounded-0" id="orderStatus2" name="orderStatus2">
  ...
  <!-- Downlaod multifile files in one click -->
  <script>
    $(document).ready(function() {
